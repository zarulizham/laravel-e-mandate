...
<body>
    <div class="font-sans antialiased text-gray-900">
        ...
        <form id="form" method="post" action="{{ $request->url }}" x-data x-init="$refs.submit.click()">
            ...
            {{-- <input type="hidden" value="{{ $request->checkSum }}" name="fpx_buyerBankBranch"> --}}
            {{-- <input type="hidden" value="{{ $request->checkSum }}" name="fpx_buyerAccNo"> --}}
            <input type="hidden" value="{{ $request->buyerId }}" name="fpx_buyerId">
            <input type="hidden" value="{{ $request->buyerMakerName }}" name="fpx_makerName">
            <input type="hidden" value="{{ $request->buyerIBAN }}" name="fpx_buyerIban">
